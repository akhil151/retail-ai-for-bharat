...
The platform architecture follows a microservices approach with:
- **Frontend**: React 18 + TypeScript with Vite build tool, using Tailwind CSS for styling and Radix UI for accessible components, TanStack Query for data fetching, Recharts for visualizations, React Router v6 for navigation, React Hook Form + Zod for form validation
- **Backend API**: Node.js + Express.js + TypeScript for REST API and business logic, JWT for authentication, Swagger/OpenAPI for API documentation
- **AI/ML Services**: Python 3.10+ based microservices using FastAPI framework for demand prediction, competitor intelligence, optimization, health scoring, leakage detection, and trend analysis. Libraries include Scikit-learn, Pandas, NumPy for data processing, Prophet/SARIMA for forecasting, spaCy/NLTK for NLP, OpenAI/Anthropic for LLM integration, Celery + Redis for task queuing
- **Data Layer**: PostgreSQL 14+ for persistent storage with TimescaleDB extension for time-series data, Redis 7+ for caching and session management, Prisma or TypeORM for database ORM
- **Real-time Processing**: Scheduled jobs and event-driven architecture for automated operations
...
- **DevOps**: Docker + Docker Compose for containerization, PM2 for process management, Winston for structured logging

The system is designed for scalability, extensibility, and real-time responsiveness while maintaining data security and user privacy.
...
graph TB
    ...
    subgraph "Application Services - Node.js/Express/TypeScript"
    ...
    subgraph "AI/ML Services - Python/FastAPI"
    ...
    subgraph "Data Layer"
        PostgreSQL[(PostgreSQL 14+ + TimescaleDB)]
    ...
    DemandPredictor --> PostgreSQL
    CompetitorTracker --> PostgreSQL
    PriceOptimizer --> PostgreSQL
    InventoryOptimizer --> PostgreSQL
    DynamicPricer --> PostgreSQL
    IntelligenceEngine --> PostgreSQL
    HealthScoreEngine --> PostgreSQL
    LeakageDetector --> PostgreSQL
    TrendAnalyzer --> PostgreSQL
...
sequenceDiagram
    ...
    WebUI->>Gateway: GET /api/pricing/recommendations/:sku
    Gateway->>PricingAPI: Request recommendations
    PricingAPI->>Database: Fetch product data
    Database-->>PricingAPI: Product + cost data
    ...
    WebUI->>Gateway: POST /api/pricing/automation/enable
    Gateway->>DynamicPricer: Enable automation for SKU
    DynamicPricer->>Database: Update automation settings
    
    Note over DynamicPricer: Scheduled job runs every 30 min
    ...
    DynamicPricer->>Database: Log price change
```
