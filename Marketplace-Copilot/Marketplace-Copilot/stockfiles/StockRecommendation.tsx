...
export default function StockRecommendation() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery<InventoryResponse>({
    ...
    queryFn: async () => {
      ...
      console.log('Inventory Optimization Response:', json);
      console.log('Items count:', json?.items?.length || 0);
      if (json?.items?.length > 0) {
        console.log('Sample item:', json.items[0]);
      }
      return json;
    },
    staleTime: 5 * 60 * 1000,
  ...
  console.log('Chart data - stockVsDemand:', stockVsDemand.length);
  console.log('Chart data - turnoverRanking:', turnoverRanking.length);
  ...
  const turnoverRanking = useMemo(() => {
    ...
    return items
      ...
      .slice(0, 15)
      .map((item) => ({ sku: item.sku.substring(0, 10), turnover: Number(item.turnover_ratio?.toFixed(2) || 0) }));
  }, [items]);
              ...
              <CardContent className="h-80">
                {stockVsDemand.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={stockVsDemand} margin={{ left: 8, right: 8, bottom: 60 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} />
                      <XAxis dataKey="sku" tick={{ fontSize: 11 }} interval={0} angle={-45} height={80} textAnchor="end" />
            ...
            <Card className="bg-card border">
              ...
              <CardContent className="h-80">
                {turnoverRanking.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={turnoverRanking} margin={{ left: 8, right: 8, bottom: 60 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} />
                      <XAxis dataKey="sku" tick={{ fontSize: 11 }} interval={0} angle={-45} height={80} textAnchor="end" />
                      <YAxis tick={{ fontSize: 11 }} />
                      <RechartsTooltip />
                      <Bar dataKey="turnover" fill="#3b82f6" radius={[4, 4, 0, 0]} name="Turnover Ratio" />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </CardContent>
